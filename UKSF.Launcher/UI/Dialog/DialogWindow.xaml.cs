...
namespace UKSF.Launcher.UI.Dialog {
    public partial class DialogWindow {
        ...
        public static MessageBoxResult Show(string title, string message, DialogBoxType type, string link) {
            switch (type) {
                ...
                case DialogBoxType.YES_NO_CANCEL: return Show(title, message, MessageBoxButton.YesNoCancel, link);
        ...
        private static MessageBoxResult Show(string title, string message, MessageBoxButton button, string link = "", UIElement control = null) {
            ...
            if (!string.IsNullOrEmpty(link)) {
                dialog.DialogMainControl.DialogMainControlTextBlock.Inlines.Clear();
                string[] parts = message.Split(new[] {"::"}, StringSplitOptions.RemoveEmptyEntries);
                dialog.DialogMainControl.DialogMainControlTextBlock.Inlines.Add(parts[0]);
                Hyperlink hyperLink = new Hyperlink {NavigateUri = new Uri(link)};
                hyperLink.Inlines.Add(link);
                hyperLink.RequestNavigate += Hyperlink_RequestNavigate;
                dialog.DialogMainControl.DialogMainControlTextBlock.Inlines.Add(hyperLink);
                foreach (string part in parts.Skip(1)) {
                    dialog.DialogMainControl.DialogMainControlTextBlock.Inlines.Add(part);
                }
            } else {
